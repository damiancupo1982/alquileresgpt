...
import { Plus, FileText, Download, Eye, Search, Filter, X, Printer, BarChart3, AlertTriangle } from 'lucide-react';
...
const ReceiptsManager: React.FC<ReceiptsManagerProps> = ({ 
  ...
  const [contractUpdateDue, setContractUpdateDue] = useState<{ due: boolean; message: string }>({ due: false, message: '' });
  ...
  const getMonthIndex = (monthName: string) => months.indexOf(monthName); // 0-11

  const isContractUpdateDue = (tenant: Tenant, monthName: string, year: number) => {
    if (!tenant.contractStart || !tenant.contractEnd) return { due: false, message: '' };
    if (!tenant.contractUpdateEveryMonths || tenant.contractUpdateEveryMonths <= 0) return { due: false, message: '' };

    const start = new Date(tenant.contractStart + 'T00:00:00');
    const end = new Date(tenant.contractEnd + 'T00:00:00');

    const monthIdx = getMonthIndex(monthName);
    if (monthIdx < 0) return { due: false, message: '' };

    const selected = new Date(year, monthIdx, 1);

    // Solo avisar si el mes está dentro del contrato
    if (selected < new Date(start.getFullYear(), start.getMonth(), 1)) return { due: false, message: '' };
    if (selected > new Date(end.getFullYear(), end.getMonth(), 1)) return { due: false, message: '' };

    const monthsSinceStart = (selected.getFullYear() - start.getFullYear()) * 12 + (selected.getMonth() - start.getMonth());
    if (monthsSinceStart <= 0) return { due: false, message: '' };

    const due = monthsSinceStart % tenant.contractUpdateEveryMonths === 0;

    if (!due) return { due: false, message: '' };

    const typeLabel = tenant.contractUpdateType === 'porcentaje' ? `${tenant.contractUpdateValue}%` : `$${tenant.contractUpdateValue.toLocaleString()}`;
    return {
      due: true,
      message: `Este mes corresponde actualización del contrato (cada ${tenant.contractUpdateEveryMonths} meses). Tipo: ${tenant.contractUpdateType}. Valor: ${typeLabel}. Corroborá los datos antes de generar el recibo.`
    };
  };
  ...
  // Aviso de actualización de contrato antes de generar recibo
  useEffect(() => {
    const selectedTenant = formData.tenantId ? tenants.find(t => t.id.toString() === formData.tenantId) : null;
    if (!selectedTenant || !formData.month) {
      setContractUpdateDue({ due: false, message: '' });
      setContractUpdateConfirmed(false);
      return;
    }
    const info = isContractUpdateDue(selectedTenant, formData.month, formData.year);
    setContractUpdateDue(info);
    setContractUpdateConfirmed(false);
  }, [formData.tenantId, formData.month, formData.year, tenants]);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();

    if (contractUpdateDue.due && !contractUpdateConfirmed) {
      alert('Este mes corresponde actualización de contrato. Confirmá que corroboraste los datos antes de generar el recibo.');
      return;
    }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {contractUpdateDue.due && (
                <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
                  <div className="flex items-start space-x-2">
                    <AlertTriangle className="h-5 w-5 text-amber-600 mt-0.5" />
                    <div className="text-sm text-amber-900">
                      <div className="font-medium">Aviso de actualización</div>
                      <div className="mt-1">{contractUpdateDue.message}</div>
                      <label className="mt-2 flex items-center space-x-2">
                        <input
                          type="checkbox"
                          checked={contractUpdateConfirmed}
                          onChange={(e) => setContractUpdateConfirmed(e.target.checked)}
                          className="h-4 w-4"
                        />
                        <span>Ya corroboré los datos para la actualización de este mes.</span>
                      </label>
                    </div>
                  </div>
                </div>
