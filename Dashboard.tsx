...
const MonthlySummary: React.FC<MonthlySummaryProps> = ({ receipts, tenants, properties, onClose }) => {
  ...
  const summaryData = useMemo(() => {
    ...
    const monthReceipts = receipts.filter(
      (receipt) => receipt.month === selectedMonth && receipt.year === selectedYear
    );

    // Mapas rápidos
    const tenantByName = new Map(tenants.map((t) => [t.name, t]));
    const receiptsByProperty = new Map<string, Receipt[]>();
    monthReceipts.forEach((r) => {
      const arr = receiptsByProperty.get(r.property) || [];
      arr.push(r);
      receiptsByProperty.set(r.property, arr);
    });

    const normalizeBuilding = (b: string) => (BUILDINGS.includes(b as any) ? b : 'Otro');

    // Orden fijo de edificios + orden fijo de propiedades dentro de cada edificio
    const orderedBuildings = [...BUILDINGS];

    const rows: SummaryRow[] = [];

    orderedBuildings.forEach((buildingName) => {
      const buildingProperties = properties
        .map((p) => ({ ...p, building: normalizeBuilding(p.building) }))
        .filter((p) => p.building === buildingName)
        .sort((a, b) => a.name.localeCompare(b.name, 'es'));

      buildingProperties.forEach((property) => {
        const tenantName = property.tenant || '';
        const tenant = tenantName ? tenantByName.get(tenantName) : undefined;

        const propReceipts = receiptsByProperty.get(property.name) || [];
        ...
        rows.push({
          ...
          paidAmount,
          totalDebt: tenant ? tenant.balance : 0
        });
      });
    });

    // Si hay propiedades con edificio vacío/no reconocido, van a "Otro" por normalizeBuilding, pero por si no estaban en properties:
    // Agregar inquilinos sin propiedad asignada (y con deuda) al final.
    tenants.forEach((t) => {
      if (!t.propertyId && t.balance > 0) {
        rows.push({
          building: 'Otro',
          address: '',
          tenant: t.name,
          property: t.property || 'Sin propiedad',
          paidAmount: 0,
          totalDebt: t.balance
        });
      }
